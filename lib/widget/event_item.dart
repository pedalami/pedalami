...
class EventItem extends StatefulWidget {
  const EventItem({Key? key, required Event event, required refresh})
      : refresh = refresh,
        event = event,
        super(key: key);
...
class _EventItemState extends State<EventItem> {
  ...
  void initState() {
    ...
    event = widget.event;
    _joined = hasJoined();
    localStartDate = DateFormat("yyyy-MM-dd HH:mm:ss")
        .parse(event.startDate.toString(), true)
        .toLocal();
    localEndDate = DateFormat("yyyy-MM-dd HH:mm:ss")
        .parse(event.endDate.toString(), true)
        .toLocal();
    super.initState();
    WidgetsBinding.instance!.addPostFrameCallback((timeStamp) async {
      if (event.isTeam()) {
        if (!_joined) {
          if (event.isPrivate()) {
            for (Team t in LoggedUser.instance!.teams!) {
              if (t.id.contains(event.hostTeamId!)) {
                joinableTeams
                    .add(await MongoDB.instance.getTeam(event.hostTeamId!));
              }
              if (t.id.contains(event.guestTeamId!)) {
                joinableTeams
                    .add(await MongoDB.instance.getTeam(event.guestTeamId!));
              }
            }
          } else {
            //load data public team events
            for (String enrolledTeamId in event.involvedTeamsIds!) {
              for (int i = 0; i < LoggedUser.instance!.teams!.length; i++) {
                if (LoggedUser.instance!.teams![i].id == enrolledTeamId) {
                  joinableTeams
                      .add(await MongoDB.instance.getTeam(enrolledTeamId));
                }
              }
              }
          }
                      ...
                      children: <Widget>[
                        ...
                        Padding(
                          padding: EdgeInsets.only(
                              top: 1 * SizeConfig.heightMultiplier!),
                          child: Text(
                            "Started: " + localStartDate.formatIT,
                            style: TextStyle(
                                color: Colors.black54,
                                fontWeight: FontWeight.bold,
                                fontSize: 2 * SizeConfig.textMultiplier!),
                            ...
                            ? Padding(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    !loadingJoinableTeams
                                        ? !_joined
                                            ? SearchChoices.single(
                                                items: item,
                                                value:
                                                    selectedValueSingleDoneButtonDialog,
                          ...
                          if (!joining) {
                            try {
                              ...
                              if (event.isIndividual()) {
                                if (await MongoDB.instance.joinEvent(event.id,
                                    FirebaseAuth.instance.currentUser!.uid)) {
                                  ScoreboardEntry s = new ScoreboardEntry(
                                      FirebaseAuth.instance.currentUser!.uid,
                                      null,
                                      0);
                                ...
                                if (selectedTeam != null) {
                                  if (await MongoDB.instance.joinEvent(event.id,
                                    ...
                                    ScoreboardEntry s = new ScoreboardEntry(
                                        ...
                                        0);
                                    event.scoreboard!.add(s);
                                    LoggedUser.instance!.joinedEvents!
                                        .add(event);
                                    setState(() {
                                      _joined = !_joined;
                                    });
