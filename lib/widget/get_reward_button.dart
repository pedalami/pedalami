...
class GetRewardButton extends StatefulWidget {
  const GetRewardButton({Key? key, required bool alreadyClicked,required Reward reward, @required notifyParent}) : reward=reward,alreadyClicked=alreadyClicked, notifyParent=notifyParent,super(key: key);
    ...
    if(LoggedUser.instance!.points!<reward.price)
    ...
    else
      {
        return ElevatedButton(
            ...
            child:!_alreadyClicked?AnimatedCrossFade(
                duration:Duration(milliseconds:100),
                crossFadeState: _alreadyClicked?CrossFadeState.showSecond:CrossFadeState.showFirst,
                firstChild:Text(price+(price=="1"?" point":" points")),
                secondChild:Text('GET')
            ):AnimatedCrossFade(
                duration:Duration(milliseconds:100),
                crossFadeState: !_alreadyClicked?CrossFadeState.showFirst:CrossFadeState.showSecond,
                firstChild:Text(price+(price=="1"?" point":" points")),
                secondChild:Text('GET')
            ),
            onPressed:(){
                    ...
                    return AlertDialog(
                      ...
                      content: SingleChildScrollView(
                        child: ListBody(
                          children: <Widget>[
                            Text(
                              "Are you sure to redeem "+reward.description+" for "+price+(price=="1"?" point":" points")+"?",
                              style: TextStyle(color: Colors.black),
                            ),
                          ],
                        ),
                        ...
                        TextButton(
                          ...
                          onPressed: () async{
                            if(await MongoDB.instance.redeemReward(reward.id)!=null)
                              ...
                              {
                                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("You don't have enough points for "+reward.description+ "!")));
                              }
